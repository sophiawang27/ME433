...
void Wheel_controller(int com){
    ...
    set_dutycycle(duty_left, duty_right);
...
void set_dutycycle(float duty_cycleL, float duty_cycleR){
    ...
    if (duty_cycleL>0.0){
        ...
        pwm_set_gpio_level(IN2_PIN, wrap*((duty_cycleL) / 100.0)); //requires 67% to start moving
    }

    else if (duty_cycleL<0.0){

        pwm_set_gpio_level(IN1_PIN, wrap*((-duty_cycleL) / 100.0)); // requires -66% to start moving
    ...
    if (duty_cycleR < 0.0){
        pwm_set_gpio_level(IN3_PIN, wrap*((duty_cycleR) / 100.0));
        pwm_set_gpio_level(IN4_PIN, 0);
    }
    else if (duty_cycleR > 0.0){
        pwm_set_gpio_level(IN3_PIN, 0);
        pwm_set_gpio_level(IN4_PIN, wrap*((-duty_cycleR) / 100.0)); 
    }
    else{
        pwm_set_gpio_level(IN3_PIN, 0);
        pwm_set_gpio_level(IN2_PIN, 0);
